...
class RunCallCenterEvaluationJob extends Command
{
    protected $signature = 'call-center:evaluate-job {jobId}';

    protected $description = 'Run a background call-center checklist evaluation job';

    public function handle(
        CallCenterEvaluationJobStore $jobStore,
        CallCenterChecklistEvaluator $evaluator,
        ...
        if ($job === null) {
            $this->error('Evaluation job not found.');
        ...
        try {
            ...
            $reporter = static function (array $event) use (
                ...
                if ($type === 'prompt') {
                    ...
                    $jobStore->appendLog($jobId, 'Збережено фактичний prompt поточного запуску Qwen.', 'status');
