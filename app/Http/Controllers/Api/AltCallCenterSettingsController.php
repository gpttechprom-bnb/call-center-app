...
class AltCallCenterSettingsController extends Controller
{
    public function show(AltCallCenterTranscriptionSettings $settings): JsonResponse
    {
    ...
    public function update(Request $request, AltCallCenterTranscriptionSettings $settings): JsonResponse
    {
        $validated = $request->validate([
            'transcription_provider' => ['nullable', Rule::in($settings->availableTranscriptionProviders())],
            'transcription_model' => ['required', 'string', 'max:255'],
        ...
        $updatedSettings = $settings->save([
            'transcription_provider' => (string) ($validated['transcription_provider'] ?? $settings->transcriptionProvider()),
        ...
        return response()->json([
            'message' => 'Налаштування alt-контуру збережено.',
            'settings' => $updatedSettings,
        ]);
    }

    public function availableModels(Request $request, AltCallCenterTranscriptionSettings $settings): JsonResponse
    {
        $kindValidated = $request->validate([
            'kind' => ['nullable', Rule::in(['llm', 'transcription'])],
        ]);
        $kind = (string) ($kindValidated['kind'] ?? 'llm');

        if ($kind === 'transcription') {
            $transcriptionValidated = $request->validate([
                'provider' => ['required', Rule::in($settings->availableTranscriptionProviders())],
            ]);

            return response()->json([
                'provider' => (string) $transcriptionValidated['provider'],
                'models' => $settings->availableTranscriptionProviderModels(
                    (string) $transcriptionValidated['provider'],
                ),
            ]);
        }
