...
class CallCenterEvaluationController extends Controller
{
    ...
    public function store(
        ...
        return response()->json([
            'message' => 'Оцінювання запущено у фоновому режимі.',
            'job' => $jobStore->publicPayload($job),
            'reused_existing_job' => false,
        ], 202);
    }

    public function show(string $jobId, CallCenterEvaluationJobStore $jobStore): JsonResponse
    {
    ...
    public function destroy(string $jobId, CallCenterEvaluationJobStore $jobStore): JsonResponse
    {
        $job = $jobStore->find($jobId);
    ...
    private function dispatchBackgroundJob(string $jobId): ?int
    {
        ...
        $command = sprintf(
            'cd %s && nohup %s %s call-center:evaluate-job %s > /dev/null 2>&1 < /dev/null & echo $!',
