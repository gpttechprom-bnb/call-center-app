...
class CallCenterTranscriptionAiRewriteController extends Controller
{
        ...
        if ((bool) ($validated['stream'] ?? false)) {
            return response()->stream(function () use ($validated, $aiRewriteService, $settings): void {
                ...
                try {
                    ...
                    $emit('completed', [
                        'message' => $result['message'] ?? 'AI-обробку тексту завершено.',
                        'text' => $result['text'],
                        'model' => $result['model'],
                    ...
                    $emit('error', [
                        'message' => 'Не вдалося виконати AI-обробку тексту. Перевірте Ollama та спробуйте ще раз.',
                    ]);
                }
            }, 200, [
